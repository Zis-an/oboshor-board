...
//                                    <button class='btn btn-danger btn-sm delete-head-item-btn' data-href='/head-items/$row->id'>Delete</button>"
    ...
    function arrange_head(Request $request, $type)
    {

        $searched_head_id = $request->head_id ?? '';

        if ($searched_head_id) {
            $head_items = HeadItem::orderBy('order', 'asc')->whereHas('head', function ($query) use ($type) {
                $query->where("type", '=', $type);
            })
                ->where('head_id', $searched_head_id)
                ->get();
        } else {
            $head_items = [];
        }
        ...
        return view('head-item.arrange_thead', compact('head_items', 'heads', 'type', 'searched_head_id'));
    }
    ...
    function edit($id)
    {
        ...
        $heads = Head::orderBy('order', 'asc')->pluck('name', 'id')->toArray();
